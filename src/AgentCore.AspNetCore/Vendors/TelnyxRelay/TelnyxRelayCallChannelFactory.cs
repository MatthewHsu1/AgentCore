...
/// <see cref="CallChannelContext"/>, and that context carries a call id and the host's own
...
internal sealed class TelnyxRelayCallChannelFactory(TelnyxRelayConnection connection) : ICallChannelFactory
{
    /// <inheritdoc />
    /// <remarks>
    ...
    /// </remarks>
    public ValueTask<CallChannel> OpenAsync(
        CallChannelContext context,
        CancellationToken cancellationToken = default)
    {
        // The same object in both slots. Nothing above this line can tell, and nothing may ask.
        return ValueTask.FromResult(new CallChannel(connection, connection));
    }
}
