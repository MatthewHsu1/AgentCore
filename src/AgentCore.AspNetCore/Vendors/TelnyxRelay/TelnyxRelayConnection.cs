...
internal sealed class TelnyxRelayConnection
{
    ...
    private void HandleInterrupt(RelayFrame.Interrupt interrupt)
    {
        ...
        lock (_turnLock)
        {
            ...
            // mark stays where it was. CallSession.Interrupt reaches the same answer from the other
            // side, because a run that has yielded nothing is not audible to it either.
