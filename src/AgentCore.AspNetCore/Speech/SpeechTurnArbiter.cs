...
/// stops handing its fragments to <see cref="ISpeechOutputPort"/>. The last gate, immediately
...
/// </para>
/// </remarks>
internal sealed class SpeechTurnArbiter
{
    ...
    private readonly ISpeechOutputPort _output;
    ...
    /// </param>
    public SpeechTurnArbiter(
        IConversationPort session,
        ISpeechOutputPort output,
    ...
    /// awaits, and it hands <see cref="ISpeechOutputPort.StopAsync"/>'s task to the observer rather
    ...
    private async Task RunTurnAsync(IConversationPort session, string text, long turnId)
    {
        try
        {
                ...
                .ConfigureAwait(false))
            {
                ...
                // That gate reads the port's own speech generation rather than this id, because no
