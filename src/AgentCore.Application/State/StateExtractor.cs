...
public sealed class StateExtractor
{
        ...
        Clarifications clarifications)
    {
        ...
        foreach (var name in SlotNames)
        {
            ...
            if (WriteSlot(state, name, value, clarifications))
            {
                filled++;
            }
            else
    ...
    /// <summary>Coerces and gates one answer through <see cref="StateDocument.TryWrite"/>.</summary>
    private static bool WriteSlot(StateDocument state, string slot, JsonNode value, Clarifications clarifications)
    {
