...
/// <summary>
/// Names the vendor behind one <c>providers.speech</c> value.
/// </summary>
/// <remarks>
/// <para>
...
/// the host lists the vendors it supports once, <c>providers.speech.kind</c> names which one the
/// document means, and a document that changes vendors changes no code.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// So <c>providers.speech.kind</c> is a <b>naming</b> field, not a selecting one: it answers "which
/// vendor did the document mean", and nothing here turns that answer into a constructed adapter.
/// <see cref="Call.CallSpeechPairing"/> is what reads the name today, and only to check it against
/// <c>providers.call.kind</c> — because a transport whose frames already carry text, such as the
/// relay this solution ships, is itself the recognizer and the synthesizer, so once the two names
...
/// frames already carry text must further be named in <b>both</b> blocks, because such a vendor
/// performs recognition and synthesis itself and a document naming it for the pipe and something
/// else for the ears describes a deployment that cannot exist.
