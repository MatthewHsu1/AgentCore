...
/// </summary>
public sealed class CallTurnArbiterTests
{
    [Fact(Timeout = 30_000)]
    ...
    {
        // A queue of held prompts would answer questions the caller has already moved past.
        var session = new ScriptedConversationPort();
        var output = new FakeCallOutput();
        var arbiter = NewArbiter(session, output);
    ...
    private static CallTurnArbiter NewArbiter(ScriptedConversationPort session, FakeCallOutput output)
        => new(
            session,
            output,
