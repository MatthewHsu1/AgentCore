...
/// <summary>
/// The two promises <see cref="ISpeechInputPort"/> makes that a signature cannot: one consumer for
...
/// </remarks>
public sealed class SpeechInputPortTests
{
    [Fact(Timeout = 30_000)]
    public async Task ASecondListenAsync_ThrowsRatherThanGivingEachReaderHalfTheCall()
    {
        var input = new FakeSpeechInput(
            new SpeechInput.Utterance("hello", "en", IsFinal: true),
            new SpeechInput.Keypress("1"),
            new SpeechInput.Barge("hel", TimeSpan.FromMilliseconds(240)));

        var heard = new List<SpeechInput>();
        ...
        Assert.Equal(new SpeechInput.Utterance("hello", "en", IsFinal: true), heard[0]);
        Assert.Equal(new SpeechInput.Keypress("1"), heard[1]);
        Assert.Equal(new SpeechInput.Barge("hel", TimeSpan.FromMilliseconds(240)), heard[2]);
    ...
    public async Task ACancelledRead_ThrowsRatherThanPassingForTheEndOfTheCall()
    {
        ...
        var input = new FakeSpeechInput(new SpeechInput.Keypress("1"));
