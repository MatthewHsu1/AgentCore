    ...
    private const string CycleYaml =
      ...
      public void AKindAgentTool_NamingAnAgentThatDoesNotExist_FailsAtLoad()
      {
          ...
          var failure = Assert.Throws<ConfigurationLoadException>(
              () => ConfigurationCompiler.CompileAll(document, new AgentCompilationContext(new FakeChatClientFactory(client)))["main"]);
      ...
      private static CompiledAgent Compile(string yaml, IChatClient client)
      {
          ...
          return ConfigurationCompiler.CompileAll(document, new AgentCompilationContext(new FakeChatClientFactory(client)))["main"];
      }
  }
